...
return the letter which comes first in the latin alphabet. For
example -- "one" contains "o", "n", "e" only once for each, thus
we choose "e".
...
def checkio(text: str) -> str:
    ...
    ary = [k for k, v in dic.items() if v == max(dic.values())]
    return min(ary)
